...
*                     SPICalutateBRG(unsigned int pb_clk, unsigned int spi_clk)
...
#if defined( SD_CS_ADDR_BIT ) // PMP Address bit conflicts with GPIO
    PMAENSET = 1 << SD_CS_ADDR_BIT; // Enable address bit
#endif//defined( SD_CS_ADDR_BIT )
    SD_CS_HI;               //Initialize Chip Select line (1 = card not selected)

    OpenSPI(SPI_START_CFG_1, SPI_START_CFG_2);
  //SPIBRG = SPICalutateBRG(GetPeripheralClock(), 2000000 );
    ...
    if(timeout == 0)
    {
        SD_CS_HI;     // Drive CS high (card not selected)
        WriteSPISlow(0xFF);       //Send some "extraneous" clock pulses.  If a previous
                                  //command was terminated before it completed normally,
                                  //the card might not have received the required clocking
                                  //following the transfer.
        SD_CS_LO;      // (card  selected)
    ...
    else
    {
        ...
        {
            mediaInformation.errorCode = MEDIA_CANNOT_INITIALIZE;
            SD_CS_HI;     //Deselect the devices
        }
    }//end else from if(((response.r7.bytewise.argument._returnVal & 0xFFF) == 0x1AA) && (!response.r7.bitwise.bits.ILLEGAL_CMD))
