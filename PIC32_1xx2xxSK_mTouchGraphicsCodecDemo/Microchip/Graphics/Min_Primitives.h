 ...
 * PAT                  05/12/2010  Added sine and cosine functions.
 * MWM                  07/13/2011  Stripped down from Primitive.c/h
 *****************************************************************************/
#ifndef _MIN_PRIMITIVES_H
    #define _MIN_PRIMITIVES_H
...
//    EDS       = 4             // EDS memory (RAM, internal PSV Flash, or external EPMP).
    FLASH        = 0,    // internal flash
    EXTERNAL     = 1,    // external memory
    EDS          = 2,    // EDS memory (RAM, internal PSV Flash, or
                         // external EPMP)
    FLASHICMP    = 3,    // compressed data in internal flash for IPU
    EXTERNALICMP = 4,    // compressed data in external memory for IPU
    EDSICMP      = 5,    // compressed data in EDS memory for IPU
    RAM          = 6,    // RAM, for cases where RAM is not EDS
    VIDEOBUF     = 7     // video buffer
} TYPE_MEMORY;
...
#ifndef COLOR_24BPP
void        BarGradient(short left, short top, short right, short bottom, WORD color1, WORD color2, BYTE direction, WORD length);
#else
void        BarGradient(short left, short top, short right, short bottom, DWORD_VAL color1, DWORD_VAL color2, BYTE direction);
#endif
WORD        BevelGradient(short x1, short y1, short x2, short y2,SHORT rad, WORD color1, WORD color2, BYTE direction,WORD length);
void        AlphaBlend(short left, short top, short right, short bottom, WORD color1, WORD color2, BYTE Amount);
