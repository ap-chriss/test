...
static UINT16 LastSliderValue  = 0;
        ...
        *pCurrentButtonStatus = mTouchCapApp_DirectKeys_Demo();
        ...
        if ( *pCurrentButtonStatus > 0 ) // Something is happening on the buttons
        {
            ...
            for ( iButton = 0; iButton < NumberScanChannels ; iButton++ )
            {
                ...
                LocalSliderValue += WeightDeviation*(iButton+1); // Buttons in natural order
              //LocalSliderValue += WeightDeviation*(NumberScanChannels-iButton); // Buttons in reversed order
            }
            // Average Weight when 256 counts is full scale (100%)
            LocalSliderValue = ((256*LocalSliderValue)/NumberScanChannels)/WeightSum;
            // Correct for 1st button, 256 counts = 100%
            LocalSliderValue = (NumberScanChannels
                                *(LocalSliderValue-FullScalePerButton))
                                  /(NumberScanChannels-1);
        }
        else // Nothing is happening, put slider to sleep
        {
            //LocalSliderValue = 0;
            LocalSliderValue = LastSliderValue;
        }
        LocalSliderValue = ( LocalSliderValue + ((1<<3) - 1)*LastSliderValue )>>3;
        LastSliderValue  = LocalSliderValue;
        *pSliderValue    = LocalSliderValue;
...
#      if defined( UART_DUMP_RAW_COUNTS ) || defined( UART_DUMP_ALL_COUNTS )
        {
            sprintf(ButtonMeasString,
                    ...
                    "%05d,%05d,%05d,%05d,"
                    "\r\n",
                    iHF_Read+1,*pCurrentButtonStatus,*pCurrentButtonAsserts,LocalSliderValue,
...
#      if defined( DUMP_DIAGNOSTICS_VIA_UART )
        sprintf( ButtonMeasString,
                 "%05d,%05d,%05d,%05d,%05d,%05d,%05d,%05d,%05d,%05d,%05d,%05d\r\n",
                 0, // Zero implies filtered data.
                 *pCurrentButtonStatus,
                 *pCurrentButtonAsserts,
                 LocalSliderValue,
                 CurrentButtonMeasurements[0],
                 CurrentAveragedMeasurements[0],
                 CurrentButtonMeasurements[1],
                 CurrentAveragedMeasurements[1],
                 CurrentButtonMeasurements[2],
                 CurrentAveragedMeasurements[2],
                 CurrentButtonMeasurements[3],
                 CurrentAveragedMeasurements[3] );
        SendDataBuffer(ButtonMeasString, strlen(ButtonMeasString) );
#      endif//defined( DUMP_DIAGNOSTICS_VIA_UART )
